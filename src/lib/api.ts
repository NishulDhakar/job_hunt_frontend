...
// Helper to deduce skills from job title for better mock data
...
export const api = {
    getJobs: async (): Promise<Job[]> => {
        try {
            ...
            return rawJobs.map((job: any) => ({
                ...
                matchScore: job.matchScore || Math.floor(Math.random() * 30) + 70, // Random score 70-100
                skills: job.skills && job.skills.length > 0 ? job.skills : inferSkills(job.title, job.description),
    ...
    getApplications: async (): Promise<Application[]> => {
        try {
            const res = await axios.get('/api/applications?userId=guest');
    ...
    chatWithAI: async (message: string) => {
        try {
            const res = await axios.post('/api/chat', { message, userId: 'guest' });
            return res.data.data;
        } catch (error) {
            return { jobs: [], explanation: "Sorry, I couldn't reach the AI service right now." };
        }
    }
};
